...
async def start_interview(req: StartRequest):
    ...
    if existing.data:
        candidate_id = existing.data[0]["id"]   # ✅ always use Supabase UUID id
    else:
        inserted = supabase.table("candidates").insert({
            "name": req.name,
            "email": req.email
        }).execute()
        candidate_id = inserted.data[0]["id"]
...
async def submit_answer(candidate_id: str, question_index: int, file: UploadFile = File(...)):
    ...
    # 1. Verify candidate exists in Supabase (✅ fix here)
    candidate_check = supabase.table("candidates").select("id").eq("id", candidate_id).execute()
    ...
    # 8. Save answer in Supabase (✅ consistent now)
...
async def finish_interview(candidate_id: str):
    ...
    # ✅ fetch all answers by candidate UUID
    res = supabase.table("interviews").select("*").eq("candidate_id", clean_id).execute()

    # ✅ delete active session
    supabase.table("sessions").delete().eq("candidate_id", clean_id).execute()

    return {"candidate_id": clean_id, "answers": res.data}
